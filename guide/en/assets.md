...
Anything CSS cannot reproduce is downloaded from the design as it is.
...
It spends no Figma requests: everything is computed from the snapshot.
...
- **`monochrome: true`** means the icon is single-colored and arrived with `currentColor`.
  Multi-colored ones come with their own colors; check that nothing was lost.
...
- **Rename on download.** The export returns layer names: `Rectangle 180`, `Снимок экрана (1) 1`,
  Cyrillic in URLs. They must land in the project as `doc-01.webp`, `hero-fields.webp`.
...
All assets are in the project, renamed and wired in; none of them drawn by hand; the paint of the
nodes carried into the CSS.
