...
import { list as listGuide, read as readGuide } from '../guide.js';
 ...
 * вещи, которых достаточно, чтобы начать: десять правил и карта разделов.
 */
const FIGMA_TOPIC = {
  figma: {
    ...
    body: () =>
      t({
        ru: `Порядок работы по макету разложен по фазам и лежит в регламенте: help с guide: имя раздела. Начинать — с guide: "index" (карта фаз), при разборе блока — guide: "block", при выборе инструмента под симптом — guide: "symptoms".

Десять правил, нарушение которых давало баги. Целиком, с разбором случаев, — guide: "rules".
...
Оговорки по конкретным инструментам сюда не входят: форма ответа, нюансы параметров, поддержка движками — это help с tool: имя.`,
        en: `The order of work on a design is laid out by phase in the handbook: help with guide: section name. Start with guide: "index" (the map of phases); when taking apart a block, guide: "block"; when choosing a tool for a symptom, guide: "symptoms".

The ten rules whose violation produced bugs. In full, with the cases behind them — guide: "rules".
  ...
  server.registerTool(
    ...
    async ({ topic, tool, guide }) => {
      if (guide) {
        ...
        return text(section.note ? `${section.text}\n\n${section.note}` : section.text);
      }
      ...
      return json({
        ...
        guide: await listGuide(),
        tools: {
          all: active ? [...active].sort() : null,
          detailed: Object.keys(DETAILS).filter(isActive),
        },
        note: t({
          ru: 'topic — общее устройство стенда. guide — регламент вёрстки по макету, порядок работы по фазам: help с guide: имя раздела. tools.all — что поднято на этом подключении, tools.detailed — у кого есть оговорки сверх описания: их отдаёт help с tool: имя.',
          en: 'topic covers how the stand works. guide is the design-to-markup handbook, the order of work by phase: help with guide: section name. tools.all is what this connection exposes, tools.detailed is which of them have caveats beyond their description: help with tool: name returns those.',
        }),
      });
    },
  );
