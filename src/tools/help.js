...
Значения. Цвет, обводку и её толщину, радиус и эффекты берут из самого узла — outline показывает их в фигурных скобках. Сводка figma_tokens говорит, какие значения в файле есть и как их назвать, но не какое из них у этого узла: подставить «1px, раз он частый» — это догадка. Тексты для контента — figma_inspect с mode: text, остальные режимы режут строки.

Проверка. figma_compare по каждому брейкпоинту: расхождение до 1px и различия рендеринга шрифтов дефектом не считаются, а общий сдвиг блока читают как одну находку — сначала чинят высоту блока выше. Раздел paint сверяет оформление, которого в текстах нет: фон, рамки, толщину линий, радиусы. Всё интерактивное проходят в сессии с animations: "allow" через browser_act — неподвижный снимок сломанного слайдера не отличается от исправного. Дальше layout_stress: длинный и пустой текст, слово без переносов, список из двенадцати, битая картинка, набор ширин. Вёрстка, не пережившая прогон, не готова, как бы точно она ни совпадала с макетом.`,
...
Values. Color, stroke and its weight, radius and effects come from the node itself — the outline shows them in curly braces. The figma_tokens summary says which values exist in the file and how to name them, not which one this node has: plugging in "1px, since it is common" is a guess. Content texts come from figma_inspect with mode: text; the other modes cut strings.

Checking. figma_compare at every breakpoint: a difference up to 1px and font rendering differences are not defects, and a whole-block shift is one finding — fix the height of the block above first. The paint section checks the styling texts do not carry: backgrounds, borders, line weights, radii. Go through everything interactive in a session with animations: "allow" using browser_act — a still screenshot of a broken slider looks the same as a working one. Then layout_stress: long and empty text, a word with no break opportunities, a list of twelve, a broken image, a range of widths. Markup that does not survive that run is not done, however exactly it matches the design.`,
      }),
  },
};
