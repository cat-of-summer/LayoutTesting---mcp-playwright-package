...
Всего инструментов: **56**. Размер манифеста: **60158** символов.
...
| `timeout` | number | Сколько ждать элемент, мс. По умолчанию 30000 |
| `force` | boolean | Кликнуть, не дожидаясь кликабельности: элемент под pointer-events: none иначе ждёт весь таймаут |
...
| `record` | boolean | Записывать совпавшие запросы: метод, адрес, заголовки и тело. У multipart — состав полей и имена файлов |
| `id` | string | Для requests: показать записи только этого правила |
| `limit` | number | Для requests: сколько последних записей показать. По умолчанию 20 |
...
**Поведение по прототипу** — _только чтение_

Что с чем связано: какая кнопка открывает какую модалку, что переключает вариант компонента, где прокрутка к якорю, что закреплено при прокрутке. Связи сгруппированы по цели — шесть одинаковых шевронов это один обработчик. Переходы отдаются готовой строкой transition.
...
| `offset` | number | С какой записи продолжить: значение из подсказки note |

## `figma_breakpoints`

**Один экран на разных ширинах** — _только чтение_

Сопоставляет кадры одного экрана разной ширины по содержимому, а не по позиции: что тот же элемент, что изменилось (кегли, отступы, направление раскладки), что исчезло, чем заменено и где разошёлся порядок чтения. Линейно меняющиеся значения отдаёт готовым clamp().

| Параметры | | |
|---|---|---|
| `figma` | array | Кадры одного экрана разной ширины: десктоп, планшет, мобильная. обязателен |
| `limit` | number | Сколько строк или записей показать |

## `figma_comments`

**Комментарии к макету** — _только чтение_

Комментарии Figma с ответами и аннотации Dev Mode, привязанные к элементам: видно, к чему относится «поправить отступ». Половина требований живёт здесь, а не в самом макете. Только канал REST: у Plugin API доступа к комментариям нет.

| Параметры | | |
|---|---|---|
| `figma` | array | Ссылки figma.com или записи ключ:id. Узлы одного файла уходят одним запросом. обязателен |
| `resolved` | boolean | Показывать и закрытые треды. По умолчанию только открытые |
| `refresh` | boolean | Спросить Figma заново, а не взять список из кэша на 5 минут |
| `limit` | number | Сколько строк или записей показать |
| `offset` | number | С какой записи продолжить: значение из подсказки note |

## `figma_compare`

**Сверстано ли как в макете**

Сравнивает страницу с кадром макета: тексты сопоставляются по содержимому, и по каждому видно смещение, размер и расхождения типографики — с селектором и id узла. Попиксельное сравнение идёт вторым слоем, картой различий. Отдельно показывает, чего на странице нет и чего нет в макете.
...
**Компоненты макета** — _только чтение_

Группирует UI-элементы кадров, чтобы не плодить классы: инстансы одного компонента и одинаковые по составу блоки сводятся в один с модификаторами. Отдельно показывает дрейф — различия в пару пикселей или полтона, которые стоит свести, и совпадения с классами проекта.

| Параметры | | |
|---|---|---|
| `figma` | array | Ссылки figma.com или записи ключ:id. Узлы одного файла уходят одним запросом. обязателен |
| `project` | object | Проект для сверки: url страницы — стенд сам заберёт её CSS, либо текст CSS или SCSS |
| `limit` | number | Сколько строк или записей показать |
| `offset` | number | С какой записи продолжить: значение из подсказки note |
...
Файлы из макета в артефакты с постоянными ссылками: render — PNG узла, высокие кадры режутся на читаемые части; svg — иконки с currentColor; image — растровые заливки, кадрированные как в макете.
...
**Узел макета** — _только чтение_

Узел макета по снимку: outline — дерево слоёв с размерами, раскладкой и текстами, одинаковые соседи свёрнуты; css — компактные стили узлов. Замена get_metadata и get_design_context без лимита вызовов.
...
**Доступ к Figma** — _только чтение_

Доступ к Figma: работает ли токен REST и сколько осталось лимита, вошёл ли стенд в редактор, не отстала ли версия API. Секретов не показывает. Первый шаг, если figma_* отказывают; action: login входит в редактор заново или дозавершает вход кодом.
...
**Структура разметки** — _только чтение_

Во что кадр превращается в разметке: дерево тегов и классов, собранное по геометрии, а не по слоям. Фон, декор и наложения выносятся отдельно, перепутанные слои переподчиняются, повторы сворачиваются в список. Рядом — слоты контента для layout_stress.

| Параметры | | |
|---|---|---|
| `figma` | string | Узел: ссылка figma.com или запись ключ:id. обязателен |
| `depth` | number | Глубина дерева. По умолчанию 10 |
| `limit` | number | Сколько строк или записей показать |
| `offset` | number | С какой записи продолжить: значение из подсказки note |
...
Снимает узлы макета в локальный снимок одним запросом на файл: десктоп, мобильную, модалки — все сразу. Дальше разбор идёт по снимку без обращений к Figma. Возвращает сводку по кадрам, канал и сколько запросов потрачено.
...
**Токены макета** — _только чтение_

Что из макета становится CSS-переменной: палитра, типографика, шкалы отступов и радиусов, тени, длительности. Показывает, сколько раз значение вбито литералом при живой переменной Figma, сводит неразличимые цвета, даёт переменные компонентов по вариантам и clamp() для значений, меняющихся по ширине.

| Параметры | | |
|---|---|---|
| `figma` | array | Ссылки figma.com или записи ключ:id. Узлы одного файла уходят одним запросом. обязателен |
| `project` | object | Проект для сверки: url страницы — стенд сам заберёт её CSS, либо текст CSS или SCSS |
| `minUses` | number | Значение становится токеном с этого числа использований. По умолчанию 2 |
...
| `include` | array | Разбирать только эти блоки — шапка и подвал иначе набивают счётчики своими находками |
| `exclude` | array | Не разбирать эти блоки |

## `layout_stress`

**Прогон контентом**

Подменяет содержимое живой страницы и смотрит, что сломается: текст длиннее и пустой, слово без переносов, список из двенадцати элементов и из одного, вертикальная и битая картинка, набор ширин. Показывает только то, что появилось от подмены, и возвращает страницу в исходное состояние.

| Параметры | | |
|---|---|---|
| `sessionId` | string | обязателен |
| `scenarios` | array | Какие сценарии гонять. По умолчанию все: text, lists, images, widths |
| `selectors` | array | Что подменять. Без них стенд выбирает сам: тексты, списки и картинки страницы |
| `factor` | number | Во сколько раз удлинять текст. По умолчанию 3 |
| `items` | number | До скольких элементов раздувать список. По умолчанию 12 |
| `widths` | array | Ширины для прогона. По умолчанию от 320 до 1440 |
| `maxItems` | number | Сколько находок показывать на сценарий. По умолчанию 20 |
...
- **`figma-layout`** — Порядок работы по макету: один снимок, разбор по снимку, вёрстка, проверка сравнением и контентом.
