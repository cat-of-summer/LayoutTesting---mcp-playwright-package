 ...
 * Число здесь не идеал, а замер: 53 456 на 2026-09-08. Смысл не в том, чтобы держать эту
 * величину, а в том, чтобы её рост был виден в diff, а не через полгода в счёте за контекст.
 */
const BUDGET = { total: 55000, perToolSchema: 3000, tools: 42 };

/*
 * Инструменты, у которых схема заметно тяжелее описания. Все семь — следствие одного и того
 * же: полный набор условий просмотра разложен в каждый из них. Список зафиксирован, чтобы
 * падало на новых случаях, а не на уже известных; он схлопнется вместе с переездом условий
 * в именованный профиль.
 */
const KNOWN_SKEW = new Set([
  'web_vitals',
  'browser_open',
  'storybook_audit',
  'audit',
