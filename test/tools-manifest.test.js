 ...
 * Число здесь не идеал, а замер: 44 568 на 2026-09-08, после переезда условий просмотра в
 * именованные профили, выноса оговорок в help и появления ресурсов (было 53 456 при 42
 * инструментах). Смысл не в
 * том, чтобы держать эту величину, а в том, чтобы её рост был виден в diff, а не через полгода
 * в счёте за контекст.
 *
 * 47 752 на 2026-09-11: закрыты семь пунктов обратной связи, и все они живут в схемах —
 * выбор файлов и ответ на диалоги в browser_act, запись запросов в browser_route, живые
 * анимации в browser_open и browser_goto, clip у screenshot, сужение области у layout_audit,
 * userAgent. Рост оплачен тем, что раньше делалось обходами через browser_eval и потому
 * стоило нескольких вызовов вместо одного.
 *
 ...
 *
 * 62 960 на 2026-09-12: группа media — image_convert (1 526) и image_placeholder (1 107). Входит в
 * core, потому что картинки оптимизируют при любой вёрстке; одна она стоит около 6 000 вместе с
 * полом.
 *
 * 65 210 на 2026-09-13: два инструмента по обратной связи с прогона вёрстки. figma_spec (около
 * 1 200) собирает спецификацию блока одним вызовом вместо связки из трёх-четырёх — рост оплачен
 * тем, что связку регулярно недобирали, и вёрстка шла по обрезанным текстам и без краски узла.
 * interaction_audit (около 1 000) проверяет интерактив как проверку: до него это делалось
 * browser_act вручную, и норму агент придумывал сам. Плюс strict у validate_html.
 *
 * 66 400 на 2026-09-15: семь параметров по разбору сдачи лендинга, каждый закрывает ошибку,
 * которую инструмент мог поймать сам: figma_sync page/offset (кадры за сотым искали перебором id),
 * figma_export parts (длинный кадр читали по шести частям из 25), figma_compare sections (pixel
 * не запускали ни разу — «высота совпала»), browser_goto frames (стартовая анимация невидима),
 * layout_audit widths (промежуточные ширины не открывали), validate_html ignore (шум библиотек
 * заслонял ошибки), help brief (чек-лист фазы перед переходом). Описания ужаты до одной строки,
 * подробности — в help с tool: имя.
 *
 ...
 */
const BUDGET = { total: 66400, perToolSchema: 2450, tools: 60 };
 ...
 *
 * matrix_run описывает девять осей перебора, crawl — семнадцать параметров обхода: схема у них
 * заведомо длиннее любого разумного описания. Здесь это принято, а не забыто.
 */
const KNOWN_SKEW = new Set(['browser_open', 'matrix_run', 'crawl']);
